...
- New `--seqgrep` parameter (backed by the official `seqkit/grep` module) to keep only `--sequences` records matching a pattern before anything else runs, e.g. to restrict a shared/combined reference set to one taxon (used by the `test_full` profile against GTDB's own multi-domain `ssu_all` distribution) (by @erikrikarddaniel) ([#20](https://github.com/nf-core/taxmarker/pull/20))
